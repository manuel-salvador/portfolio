    ...
    <div className="mt-20 flex min-h-[calc(100vh-5rem)] w-full flex-col items-center justify-center gap-8 px-4 pb-10">
      ...
      <ul className="flex w-full max-w-xs flex-col gap-4">
        {links.map((link) => (
          <li key={link.slug}>
            <a
              className="block w-full cursor-pointer rounded-md border-2 border-gray-200 p-2 text-center text-lg transition-all duration-300 hover:scale-105 hover:shadow-gray-400 hover:shadow-md"
              href={`${link.path}`}
              target="_blank"
            >
              {link.slug}
